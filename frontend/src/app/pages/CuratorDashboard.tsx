...
export const CuratorDashboard: React.FC = () => {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        toast.error(error instanceof Error ? error.message : 'Failed to load curator data');
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handleModerateOpportunity = async (opportunityId: string, status: 'active' | 'closed') => {
    try {
      ...
      toast.success('Opportunity moderated');
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Failed to moderate opportunity');
    }
  };
      ...
      <div className="container mx-auto px-4 py-6">
        <Tabs defaultValue="verification">
          ...
          <TabsContent value="verification">
            <Card>
              <CardHeader><CardTitle>Pending Companies</CardTitle></CardHeader>
              <CardContent className="space-y-3">
                {isLoading ? <p className="text-gray-500">Loading...</p> : pendingCompanies.map((company) => (
                  <div key={company.id} className="border rounded-lg p-4">
                    <h4 className="font-semibold">{company.name}</h4>
                    <p className="text-sm text-gray-500">{company.email}</p>
                    <Button className="mt-3" onClick={() => void handleVerifyCompany(company.id)}><CheckCircle className="w-4 h-4 mr-2" />Verify</Button>
                  </div>
                ))}
              </CardContent>
            </Card>
                ...
                <div className="space-y-3">
                  {companies.filter((company) => company.name.toLowerCase().includes(searchQuery.toLowerCase())).map((company) => (
                    <div key={company.id} className="border rounded-lg p-4 flex justify-between">
                      ...
                      <Badge>{company.verified ? 'verified' : 'pending'}</Badge>
                    </div>
                  ))}
                </div>
              </CardContent>
              ...
              <CardContent className="space-y-3">
                {[...applicants, ...employers].map((user) => (
                  <div key={user.id} className="border rounded-lg p-4 flex justify-between">
                    ...
                    <Button variant="destructive" onClick={() => void handleBlockUser(user.id)}><XCircle className="w-4 h-4 mr-2" />Block</Button>
                  </div>
                ))}
              </CardContent>
            </Card>
          ...
          <TabsContent value="opportunities">
            <Card>
              <CardHeader><CardTitle>Pending Opportunities</CardTitle></CardHeader>
              <CardContent className="space-y-3">
                {pendingOpportunities.map((opportunity) => (
                  <div key={opportunity.id} className="border rounded-lg p-4">
                    ...
                    <div className="flex gap-2 mt-3">
                      <Button onClick={() => void handleModerateOpportunity(opportunity.id, 'active')}><CheckCircle className="w-4 h-4 mr-2" />Approve</Button>
                      <Button variant="destructive" onClick={() => void handleModerateOpportunity(opportunity.id, 'closed')}><XCircle className="w-4 h-4 mr-2" />Reject</Button>
                    </div>
                  </div>
                ))}
              </CardContent>
          ...
          <TabsContent value="curators">
            <Card>
              <CardHeader><CardTitle>Curators</CardTitle></CardHeader>
              <CardContent className="space-y-3">
