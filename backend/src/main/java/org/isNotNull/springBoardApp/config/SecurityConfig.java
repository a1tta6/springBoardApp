package org.isNotNull.springBoardApp.config;

import jakarta.servlet.http.Cookie;
import jakarta.servlet.http.HttpServletResponse;
import lombok.AllArgsConstructor;
import org.isNotNull.springBoardApp.auth.service.OAuth2UserService;
import org.isNotNull.springBoardApp.auth.util.OAuth2LoginSuccessHandler;
import org.isNotNull.springBoardApp.config.jwt.JwtAuthenticationEntryPoint;
import org.isNotNull.springBoardApp.config.jwt.JwtRequestFilter;
...
public class SecurityConfig {
    ...
    private static final String V1_OPPORTUNITIES = "/v1/opportunities";
    private static final String V1_OPPORTUNITIES_ID = "/v1/opportunities/{id}";
    private static final String V1_OPPORTUNITIES_EMPLOYERS_ID = "/v1/opportunities/employers/{id}";
    private static final String V1_OPPORTUNITIES_FAVORITES = "/v1/opportunities/favorites";
    private static final String V1_OPPORTUNITIES_ID_FAVORITE = "/v1/opportunities/{id}/favorite";
    private static final String V1_CURATOR_ALL = "/v1/curator/**";
    ...
    private static final String V1_OPPORTUNITIES_ID_RESPONSES = "/v1/opportunities/{id}/responses/**";
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers(HttpMethod.GET, V1_USERS_MEMBERS_ID).hasAnyRole(RoleEnum.MODERATOR.name(), RoleEnum.MEMBER.name(), RoleEnum.ORGANIZER.name())
                        ...
                        // Opportunity responses (Trampoline)
                        .requestMatchers(HttpMethod.GET, "/v1/opportunities/responses/me").hasRole(RoleEnum.MEMBER.name())
                        .requestMatchers(HttpMethod.POST, "/v1/opportunities/{id}/responses").hasRole(RoleEnum.MEMBER.name())
                        .requestMatchers(HttpMethod.GET, "/v1/opportunities/{id}/responses").hasAnyRole(RoleEnum.ORGANIZER.name(), RoleEnum.MODERATOR.name())
                        .requestMatchers(HttpMethod.PATCH, "/v1/opportunities/{id}/responses/{applicantId}").hasAnyRole(RoleEnum.ORGANIZER.name(), RoleEnum.MODERATOR.name())

                        // Opportunities (Trampoline) - public browse + employer manage
                        .requestMatchers(HttpMethod.GET, V1_OPPORTUNITIES).permitAll()
                        .requestMatchers(HttpMethod.GET, V1_OPPORTUNITIES_ID).permitAll()
                        .requestMatchers(HttpMethod.GET, V1_OPPORTUNITIES_EMPLOYERS_ID).permitAll()
                        .requestMatchers(HttpMethod.POST, V1_OPPORTUNITIES).hasRole(RoleEnum.ORGANIZER.name())
                        .requestMatchers(HttpMethod.PATCH, V1_OPPORTUNITIES_ID).hasAnyRole(RoleEnum.ORGANIZER.name(), RoleEnum.MODERATOR.name())
                        .requestMatchers(HttpMethod.DELETE, V1_OPPORTUNITIES_ID).hasAnyRole(RoleEnum.ORGANIZER.name(), RoleEnum.MODERATOR.name())

                        // Favorites (authorized users)
                        .requestMatchers(HttpMethod.GET, V1_OPPORTUNITIES_FAVORITES).hasRole(RoleEnum.MEMBER.name())
                        .requestMatchers(HttpMethod.POST, V1_OPPORTUNITIES_ID_FAVORITE).hasRole(RoleEnum.MEMBER.name())
                        .requestMatchers(HttpMethod.DELETE, V1_OPPORTUNITIES_ID_FAVORITE).hasRole(RoleEnum.MEMBER.name())

                        // Curator cabinet
                        .requestMatchers(V1_CURATOR_ALL).hasRole(RoleEnum.MODERATOR.name())
